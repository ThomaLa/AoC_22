    ...
    for (neigh_1, dist_1) in pairs(paths[pos_1])
      ...
      for (neigh_2, dist_2) in pairs(paths[pos_2])
        ...
        if max(new_time_1, new_time_2) < 26 && neigh_1 != neigh_2
          explore_together(
            flow + (25 - time_1 - dist_1) * valves[neigh_1].flow_rate + (
                    25 - time_2 - dist_2) * valves[neigh_2].flow_rate,
            union(open_valves, [neigh_1, neigh_2]),
            new_time_1 + 1, neigh_1,
            new_time_2 + 1, neigh_2)
        elseif new_time_1 < 26
          explore_together(
            flow + (25 - time_1 - dist_1) * valves[neigh_1].flow_rate,
            union(open_valves, [neigh_1]),
            new_time_1 + 1, neigh_1,
            time_2, pos_2)
        elseif new_time_2 < 26
          explore_together(
            flow + (25 - time_2 - dist_2) * valves[neigh_2].flow_rate,
            union(open_valves, [neigh_2]),
            time_1, pos_1,
            new_time_2 + 1, neigh_2)
        end
      end
